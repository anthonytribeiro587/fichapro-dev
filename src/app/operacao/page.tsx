...
function OperacaoContent() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (empresaError) {
      setError('Execute a migration v53 no Supabase DEV para ativar a central de operação.');
      setLoading(false);
      return;
    }
    ...
    if (tarefasResult.error) setError('Não foi possível carregar as próximas ações. Verifique as migrations v53 e v54 no Supabase DEV.');
  ...
  const metrics = useMemo(() => {
    ...
    const abertas = tarefas.filter((task) => isOpen(task.status));
    return {
      abertas,
      hoje: abertas.filter((task) => task.data_limite === today),
      atrasadas: abertas.filter((task) => Boolean(task.data_limite && task.data_limite < today)),
      prioritarias: abertas.filter((task) => task.prioridade === 'alta' || task.prioridade === 'urgente')
    };
  }, [tarefas]);

  const filteredTasks = useMemo(() => {
    if (filter === 'abertas') return tarefas.filter((task) => isOpen(task.status));
    if (filter === 'concluidas') return tarefas.filter((task) => task.status === 'concluida');
    return tarefas;
  }, [filter, tarefas]);
  ...
  async function createTask(event: FormEvent<HTMLFormElement>) {
    ...
    if (insertError) setError(insertError.message);
      ...
      setForm(initialForm);
      setMessage('Próxima ação criada.');
      await loadData();
    }
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <div><span className={styles.eyebrow}>Central de operação</span><h2>Do pagamento à próxima ação</h2><p>Uma fila única para separar, entregar, renovar, liberar, cobrar ou acompanhar.</p></div>
        <div className={styles.heroActions}><div className={styles.companyCard}><span>Empresa atual</span><strong>{empresa?.nome || 'Carregando...'}</strong><small>{empresa?.perfil_negocio || 'comércio'}</small></div><Link href="/configuracoes/negocio">Configurar negócio</Link></div>
      </section>

      <section className={styles.metrics}>
        <article><span>Abertas</span><strong>{loading ? '...' : metrics.abertas.length}</strong><small>aguardando execução</small></article>
        <article><span>Hoje</span><strong>{loading ? '...' : metrics.hoje.length}</strong><small>com prazo hoje</small></article>
        <article className={styles.warning}><span>Atrasadas</span><strong>{loading ? '...' : metrics.atrasadas.length}</strong><small>precisam de atenção</small></article>
        <article className={styles.priority}><span>Prioritárias</span><strong>{loading ? '...' : metrics.prioritarias.length}</strong><small>altas ou urgentes</small></article>
      </section>
      ...
      <div className={styles.workspace}>
        <section className={styles.taskPanel}>
          ...
          <div className={styles.taskList}>
            {loading && <Notice>Carregando próximas ações...</Notice>}
            {!loading && filteredTasks.length === 0 && <div className={styles.empty}>Nenhuma ação encontrada.</div>}
            {filteredTasks.map((task) => {
              const overdue = Boolean(task.data_limite && task.data_limite < todayISO() && isOpen(task.status));
              return <article key={task.id} className={`${styles.taskCard} ${overdue ? styles.overdue : ''}`}>
                <div className={styles.taskMain}><span className={styles.avatar}>{initials(task.clientes?.nome || typeLabel[task.tipo])}</span><div><div className={styles.taskTitleLine}><strong>{task.titulo}</strong><span className={`${styles.badge} ${styles[task.prioridade]}`}>{priorityLabel[task.prioridade]}</span></div><p>{task.clientes?.nome || 'Sem cliente'} · {typeLabel[task.tipo]}</p>{task.descricao && <small>{task.descricao}</small>}</div></div>
                <div className={styles.taskMeta}><span className={`${styles.badge} ${styles[task.status]}`}>{statusLabel[task.status]}</span><span className={overdue ? styles.dueOverdue : ''}>{task.data_limite ? `${overdue ? 'Atrasada: ' : 'Prazo: '}${formatDate(task.data_limite)}` : 'Sem prazo'}</span></div>
                <div className={styles.taskActions}>{task.status === 'pendente' && <button type="button" onClick={() => updateStatus(task, 'em_andamento')}>Iniciar</button>}{isOpen(task.status) && <button type="button" className={styles.primaryAction} onClick={() => updateStatus(task, 'concluida')}>Concluir</button>}{task.status === 'concluida' && <button type="button" onClick={() => updateStatus(task, 'pendente')}>Reabrir</button>}</div>
              </article>;
            })}
          </div>
        </section>

        <aside className={styles.createPanel}><span className={styles.eyebrow}>Registro rápido</span><h3>Nova próxima ação</h3><p>Agora é manual. Depois, Mercado Pago, Evolution e IA poderão alimentar esta fila.</p>
          <form onSubmit={createTask} className={styles.form}>
            <label>O que precisa ser feito?<input required value={form.titulo} onChange={(event) => setForm((current) => ({ ...current, titulo: event.target.value }))} placeholder="Ex.: Renovar acesso" /></label>
            <div className={styles.formGrid}><label>Tipo<select value={form.tipo} onChange={(event) => setForm((current) => ({ ...current, tipo: event.target.value as TarefaTipo }))}>{taskTypes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>Prioridade<select value={form.prioridade} onChange={(event) => setForm((current) => ({ ...current, prioridade: event.target.value as TarefaPrioridade }))}><option value="baixa">Baixa</option><option value="normal">Normal</option><option value="alta">Alta</option><option value="urgente">Urgente</option></select></label></div>
            ...
            <label>Observação<textarea rows={3} value={form.descricao} onChange={(event) => setForm((current) => ({ ...current, descricao: event.target.value }))} /></label>
            <button type="submit" disabled={saving || !empresa}>{saving ? 'Salvando...' : 'Criar próxima ação'}</button>
          </form>
        </aside>
      </div>
    </div>
