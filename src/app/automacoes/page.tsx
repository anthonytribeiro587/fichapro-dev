...
const emptyConnection: ConnectionState = { loading: true, ok: false, configured: false, label: 'Verificando', detail: 'Aguarde...', environment: 'unknown' };
...
function AutomacoesContent() {
  ...
  const testConnections = useCallback(async () => {
    ...
    try {
      const [evolutionResponse, mpResponse] = await Promise.all([
        authFetch('/api/integracoes/evolution/webhook'),
        authFetch('/api/integracoes/mercadopago/status')
      ]);
      const [evolutionPayload, mpPayload] = await Promise.all([
        evolutionResponse.json().catch(() => ({})),
        mpResponse.json().catch(() => ({}))
      ]);

      const state = String(evolutionPayload?.connection?.instance?.state || evolutionPayload?.connection?.state || '').toLowerCase();
      const evolutionConnected = evolutionResponse.ok && ['open', 'connected', 'online'].includes(state);
      setEvolution({
        loading: false,
        ok: evolutionConnected,
        configured: evolutionResponse.status !== 503,
        label: evolutionConnected ? 'Conectado' : 'Precisa de atenção',
        detail: evolutionConnected ? 'Instância online e pronta para mensagens.' : evolutionPayload?.error || `Estado atual: ${state || 'não identificado'}`
      });

      const mpOk = mpResponse.ok && mpPayload.ok;
      const environment = mpPayload.environment === 'test' ? 'test' : mpPayload.environment === 'production' ? 'production' : 'unknown';
      setMercadoPago({
        loading: false,
        ok: mpOk,
        configured: mpPayload.configured !== false,
        label: mpOk ? (environment === 'test' ? 'Teste conectado' : 'Conectado') : 'Precisa configurar',
        detail: mpOk
          ? environment === 'test'
            ? `Conta ${mpPayload.account?.nickname || 'TESTUSER'} validada com credencial de teste. Troque pelo token de produção para gerar Pix real.`
            : `Conta ${mpPayload.account?.nickname || mpPayload.account?.email || 'Mercado Pago'} validada.`
          : mpPayload.error || 'Credencial não validada.',
        environment
      });
    } catch (connectionError) {
      const detail = connectionError instanceof Error ? connectionError.message : 'Não foi possível consultar as integrações.';
      setEvolution({ loading: false, ok: false, configured: false, label: 'Indisponível', detail });
      setMercadoPago({ loading: false, ok: false, configured: false, label: 'Indisponível', detail, environment: 'unknown' });
  ...
  async function createFromTemplate(template: Template) {
    ...
    try {
      ...
      setError(templateError instanceof Error ? templateError.message : 'Não foi possível adicionar o modelo.');
    } finally {
      setActionKey(null);
    }
  }
  ...
  async function configureEvolutionWebhook() {
    if (!empresa || actionKey === 'evolution-webhook') return;
    setActionKey('evolution-webhook');
    setIntegrationError(null);
    setIntegrationMessage('Enviando a URL do FichaPRO para a Evolution...');
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || 'Não foi possível configurar o webhook.');

      setIntegrationMessage(`Webhook configurado e conferido. ${payload.webhookUrl || ''}`.trim());
      await Promise.all([loadData(), testConnections()]);
      setIntegrationMessage('Webhook da Evolution configurado. Agora envie uma mensagem de outro número para validar a entrada.');
    } catch (webhookError) {
      setIntegrationMessage(null);
      setIntegrationError(webhookError instanceof Error ? webhookError.message : 'Não foi possível configurar o webhook.');
    } finally {
      setActionKey(null);
    }
  }
    ...
    <div className={styles.page}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroCopy}>
          ...
          <div className={styles.heroActions}><button type="button" onClick={() => setTab('regras')}>Ver minhas regras</button><Link href="/pagamentos">Abrir pagamentos</Link></div>
        </div>
        <div className={styles.setupCard}>
          <div className={styles.setupHeader}><div><span>Implantação</span><strong>{setupSteps.filter((step) => step.done).length} de {setupSteps.length} etapas</strong></div><b>{Math.round((setupSteps.filter((step) => step.done).length / setupSteps.length) * 100)}%</b></div>
          <div className={styles.progress}><span style={{ width: `${(setupSteps.filter((step) => step.done).length / setupSteps.length) * 100}%` }} /></div>
          <div className={styles.stepList}>{setupSteps.map((step) => <Link href={step.href} key={step.label} className={step.done ? styles.done : ''}><i>{step.done ? '✓' : '○'}</i><span>{step.label}</span><small>{step.done ? 'pronto' : 'configurar'}</small></Link>)}</div>
        </div>
      </section>
      ...
      {tab === 'visao' && <>
        ...
        <section className={styles.section} id="modelos">
          ...
          <div className={styles.templateGrid}>{templates.map((template) => <article className={styles.templateCard} key={template.key}><div className={styles.templateTop}><span className={styles.templateIcon}>{template.icon}</span>{template.premium && <b>Premium</b>}</div><h4>{template.title}</h4><p>{template.description}</p><div className={styles.flowMini}><span>{template.trigger.replaceAll('_', ' ')}</span><i>→</i><strong>{template.result}</strong></div><button type="button" disabled={actionKey === template.key} onClick={() => createFromTemplate(template)}>{actionKey === template.key ? 'Adicionando...' : 'Usar este modelo'}</button></article>)}</div>
        </section>
      </>}
      ...
      {tab === 'regras' && <section className={styles.section}>
        ...
        <div className={styles.rulesList}>
          ...
          {automations.map((item) => <article className={styles.ruleCard} key={item.id}><div className={`${styles.ruleStatus} ${styles[item.status]}`}><i /></div><div className={styles.ruleCopy}><div><strong>{item.nome}</strong><span className={`${styles.statusPill} ${styles[item.status]}`}>{item.status}</span></div><p>{item.descricao || 'Sem descrição.'}</p><small>Quando <b>{item.gatilho.replaceAll('_', ' ')}</b> → canal <b>{item.canal}</b></small></div><div className={styles.ruleActions}>{item.status !== 'ativa' && <button type="button" disabled={actionKey === item.id} onClick={() => changeStatus(item, 'ativa')}>Ativar</button>}{item.status === 'ativa' && <button type="button" disabled={actionKey === item.id} onClick={() => changeStatus(item, 'pausada')}>Pausar</button>}{item.status === 'pausada' && <button type="button" className={styles.ghost} onClick={() => changeStatus(item, 'rascunho')}>Voltar a rascunho</button>}</div></article>)}
        </div>
      </section>}

      {tab === 'integracoes' && <section className={styles.section} id="integracoes">
        <div className={styles.sectionHead}><div><span className={styles.kicker}>Conexões do negócio</span><h3>Integrações e webhooks</h3><p>Conexão online não significa webhook configurado. Use os botões abaixo e acompanhe o resultado nesta mesma área.</p></div><button type="button" className={styles.textButton} onClick={testConnections}>Testar novamente</button></div>
        {integrationError && <Notice type="danger">{integrationError}</Notice>}
        {integrationMessage && <Notice type="success">{integrationMessage}</Notice>}
        ...
        <div className={styles.securityNote}><span>🔒</span><div><strong>Credenciais protegidas no servidor</strong><p>A próxima ação só será criada após uma mensagem real recebida ou um pagamento efetivamente confirmado.</p></div></div>
      </section>}
      ...
      {tab === 'historico' && <section className={styles.section}>
        ...
        <div className={styles.historyList}>{!loading && executions.length === 0 && <div className={styles.emptyState}><span>◷</span><h4>Ainda não há execuções</h4><p>Os registros aparecerão aqui quando os webhooks e regras começarem a processar eventos.</p></div>}{executions.map((item) => <article key={item.id}><span className={`${styles.historyDot} ${styles[item.status]}`} /><div><strong>{item.automacoes?.nome || item.evento_origem.replaceAll('_', ' ')}</strong><small>{new Date(item.created_at).toLocaleString('pt-BR')}</small></div><span className={`${styles.statusPill} ${styles[item.status]}`}>{item.status}</span>{item.erro && <p>{item.erro}</p>}</article>)}</div>
      </section>}
